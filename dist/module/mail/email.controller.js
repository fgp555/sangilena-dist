...
class EmailController {
    ...
    async feedback(req, res, next) {
        try {
            await service.sendFeedbackEmail(req.body);
            res.json({ success: true, message: "¡Feedback enviado con éxito!" });
        }
        catch (error) {
            console.error("Error al enviar feedback:", error);
            next(new error_middleware_1.AppError("Error al enviar el feedback", 500));
        }
    }
}
